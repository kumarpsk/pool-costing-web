...
function CostBreakdownChart({ 
  ...
  pumpRoomCost = 0,
  equipmentDistance = 0,
  useDollars = false  // Changed from 'currency' to 'useDollars'
}) {
  ...
  // UPDATED: Currency formatting functions using useDollars prop
  ...
  const formatCompact = (amount) => {
    const displayAmount = useDollars ? amount / EXCHANGE_RATE : amount;
    const symbol = useDollars ? '$' : '₹';
  ...
  const CurrencyDisplay = () => (
    <div style={{
      ...
      <span style={{ 
        ...
        Currency: {getCurrencySymbol()} {getCurrencyCode()}
      </span>
      {useDollars && (
        <span style={{ 
