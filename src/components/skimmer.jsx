...
function Skimmer() {
  const [form, setForm] = useState({
    ...
    standardSize: "",
    hasSlope: false,
    poolType: "in-ground",  // This now correctly represents construction type (in-ground/terrace)
    turnover: "4.5"
  });
    ...
    if (savedDimensions) {
      try {
        ...
        const dimensionsWithType = parsed.map(dim => ({
          ...dim,
          poolType: dim.poolType || "in-ground",  // This stores construction type
  ...
  const saveToRecentDimensions_6 = (dimensions) => {
    const newDimension = {
      id: Date.now(),
      ...dimensions,
      poolType: form.poolType,  // Store construction type (in-ground/terrace)
    ...
    const updatedDimensions = [
      ...
      ...recentDimensions_6.filter(d =>
        ...
        d.poolType !== form.poolType ||
        d.turnover !== form.turnover
      ).slice(0, 4)
    ];
  ...
  const validateForm_6 = () => {
    const newErrors = {};
    if (!form.turnover || parseFloat(form.turnover) <= 0) {
      newErrors.turnover = "Please enter a valid turnover (hours)";
    }
    if (showStandardSizes_6) {
      if (!form.standardSize) newErrors.standardSize = "Please select a standard pool size";
    } else {
      if (!form.length || form.length <= 0) newErrors.length = "Please enter a valid length";
      if (!form.width || form.width <= 0) newErrors.width = "Please enter a valid width";
      if (form.hasSlope) {
        if (!form.shallowDepth || form.shallowDepth <= 0) newErrors.shallowDepth = "Please enter a valid shallow depth";
        if (!form.deepDepth || form.deepDepth <= 0) newErrors.deepDepth = "Please enter a valid deep depth";
        if (parseFloat(form.shallowDepth) >= parseFloat(form.deepDepth)) {
          newErrors.deepDepth = "Deep end must be deeper than shallow end";
        }
      } else {
        if (!form.depth || form.depth <= 0) newErrors.depth = "Please enter a valid depth";
      }
    }
  ...
  const handleChange_6 = (e) => {
    ...
    if (name === "standardSize") {
      ...
      if (selected) {
        setForm({ ...form, standardSize: value, length: selected.length, width: selected.width, depth: selected.depth, hasSlope: false, turnover: form.turnover });
      } else {
        setForm({ ...form, standardSize: value });
      }
  ...
  const handleSizeCardClick_6 = (size) => {
    setForm({ ...form, standardSize: size.label, length: size.length, width: size.width, depth: size.depth, hasSlope: false, turnover: form.turnover });
  };
  ...
  const calculateAndShowResults_6 = async (dimensions) => {
    ...
    try {
      ...
      // Backend expects pool_type for construction type (terrace/in-ground)
      const payload = {
        length: dimensions.length,
        width: dimensions.width,
        depth: dimensions.depth,
        pool_type: dimensions.poolType || form.poolType,  // Backend API still gets terrace/in-ground
        turnover: parseFloat(form.turnover),
        include_heat_pump: true,
        include_pump_room: false
      };
      
      const response = await fetch(`${API_BASE_URL}/skimmer/calculate`, {
        method: "POST",
        headers: headers,
      ...
      // ✅ FIXED: Navigate with separated poolType and constructionType
      navigate("/result", {
        state: {
          result: data,
          dimensions: dimensions,
          
          // System type - always "skimmer" for this component
          poolType: "skimmer",
          
          // Construction type - separate from pool system type
          constructionType: dimensions.poolType || form.poolType,
          
          turnover: form.turnover
        }
      });
      
      setForm({ length: "", width: "", depth: "", shallowDepth: "", deepDepth: "", standardSize: "", hasSlope: false, poolType: form.poolType, turnover: form.turnover });
    } catch (error) {
  ...
  const handleRecentDimensionClick_6 = async (dimension) => {
    setForm({ 
      ...
      poolType: dimension.poolType || "in-ground",  // Construction type
  ...
  const handleSubmit_6 = async (e) => {
    ...
    try {
      let dimensions;
      if (showStandardSizes_6) {
        const selected = Object.values(standardSizes_6).flat().find(size => size.label === form.standardSize);
        dimensions = { 
          length: selected.length, 
          width: selected.width, 
          depth: selected.depth, 
          hasSlope: false, 
          poolType: form.poolType,  // Construction type (in-ground/terrace)
          turnover: parseFloat(form.turnover) 
        };
      } else {
        dimensions = { 
          ...
          hasSlope: form.hasSlope, 
          poolType: form.poolType,  // Construction type (in-ground/terrace)
          turnover: parseFloat(form.turnover) 
        };
        if (form.hasSlope) {
          dimensions.shallowDepth = parseFloat(form.shallowDepth);
          dimensions.deepDepth = parseFloat(form.deepDepth);
          dimensions.depth = (dimensions.shallowDepth + dimensions.deepDepth) / 2;
        } else {
          dimensions.depth = parseFloat(form.depth);
          dimensions.shallowDepth = parseFloat(form.depth);
          dimensions.deepDepth = parseFloat(form.depth);
        }
      }
      saveToRecentDimensions_6({ ...dimensions, poolType: form.poolType, turnover: form.turnover });
    ...
    if (saved && saved.result) {
      ...
      navigate("/result", { 
        state: { 
          ...
          poolType: "skimmer",  // System type
          constructionType: saved.poolType || "in-ground",  // Construction type
    ...
    return (
      <div className="auth-required-container_6">
        <div className="auth-required-card_6">
          ...
          <p>Please login to access the Pool Cost Calculator</p>
    ...
    <div className="app-wrapper_6">
      ...
      <aside className="sidebar_6">
        <div className="sidebar-header_6">
          <h1>Skimmer Pool Size Selection</h1>
          <p>Choose between standard pool sizes or enter custom dimensions</p>
        </div>

        {/* ✅ Construction Type Selection - Separate from pool system type */}
          ...
          <div className="radio-group_6">
            <label className="radio-label_6">
              <input 
                ...
                name="poolType" 
                value="in-ground" 
                checked={form.poolType === "in-ground"} 
        ...
        <div className="size-toggle-container-sidebar_6">
          ...
          <button 
            onClick={() => { setShowStandardSizes_6(false); setForm(p => ({ ...p, standardSize: "" })); }} 
            className={`size-toggle-button-sidebar_6 ${!showStandardSizes_6 ? "active_6" : ""}`}
          >
              ...
              {recentDimensions_6.map((dimension) => (
                <div key={dimension.id} className="recent-item_6" onClick={() => handleRecentDimensionClick_6(dimension)}>
                  <div className="recent-dimension-info_6">
                    ...
                    <span className="recent-pool-type_6">
                      {dimension.poolType === "terrace" ? "🏢 Terrace" : "⛰️ In-Ground"}
                    </span>
                  </div>
              ...
              {savedResults_6.map((item) => (
                <li key={item.id} className="saved-list-item_6">
                  <div className="saved-result-info_6">
                    ...
                    <div className="saved-pool-type_6"><strong>Type:</strong> {item.poolType === "terrace" ? "🏢 Terrace" : "⛰️ In-Ground"}</div>
        ...
        <div className="sidebar-info_6">
          ...
          <div className="info-links_6">
            <a href="/210805-Facilities-Rules_clean.pdf" className="info-link_6" target="_blank" rel="noopener noreferrer">
              <span className="link-icon_6">📄</span>Swimming Pool Facilities Rules
            </a>
            <a href="/swimming pool products.pdf" className="info-link_6" target="_blank" rel="noopener noreferrer">
              <span className="link-icon_6">🔧</span>Pool Equipment Catalog
            </a>
            <a href="/Expanded Pool Leak Detection Guide.pdf" className="info-link_6" target="_blank" rel="noopener noreferrer">
              <span className="link-icon_6">🏢</span>About Swimming Pool Leak Detection
            </a>
            <Link to="/heatpump" className="info-link_6">
              <span className="link-icon_6">🌡️</span>Heat Pump Calculator
            </Link>
          </div>
        </div>
      </aside>
      ...
      <main className="app-container_1_6">
        ...
        <div className="bg-orb_6 bg-orb-1_6"></div>
        <div className="bg-orb_6 bg-orb-2_6"></div>
        <div className="bg-orb_6 bg-orb-3_6"></div>

        <div className="app-header_6">
          <div className="header-eyebrow_6">POOL CONSTRUCTION ESTIMATOR</div>
          <h2 className="app-title_6">Skimmer Pool Cost Estimator</h2>
          <p className="app-subtitle_6">Calculate your pool construction costs accurately</p>
          {/* ✅ Display construction type, not pool system type */}
          <div className="current-pool-type_6">
            <span className={`pool-type-badge_6 ${form.poolType}`}>
              {form.poolType === "terrace" ? "🏢 Terrace Pool" : "⛰️ In-Ground Pool"}
            </span>
          </div>
        </div>
            ...
            <div className="standard-sizes-container_6">
              ...
              <div className="sizes-grid_6">
                {standardSizes_6[selectedCategory_6].map((size, idx) => (
                  <div 
                    key={idx} 
                    className={`size-card_6 ${form.standardSize === size.label ? "selected_6" : ""}`} 
              ...
              {form.standardSize && (
                <div className="selected-size-display_6">
                  ...
                  <strong>Selected: {form.standardSize}</strong>
                </div>
              )}
                ...
                >
                  ...
                  {Object.values(standardSizes_6).flat().map((size, idx) => (
                    <option key={idx} value={size.label}>
                      {size.label} - {size.area} ({size.type})
                    </option>
                  ))}
                </select>
              ...
              <div className="slope-toggle_6">
                <label className="toggle-label_6">
                  <input 
                    type="checkbox" 
                    name="hasSlope" 
                    checked={form.hasSlope} 
                    onChange={handleChange_6} 
                  />
                  <span className="toggle-text_6">
                    <span className="toggle-icon_6">↗️</span>
                ...
                <div className="input-row_6">
                  <div className="input-group_6">
                    ...
                    <input 
                      ...
                      step="0.1" 
                      min="0.5" 
                      value={form.shallowDepth} 
                      onChange={handleChange_6} 
                      placeholder="e.g. 1.0" 
                      className={`form-input_6 ${errors_6.shallowDepth ? 'error_6' : ''}`} 
                    />
                  ...
                  <div className="input-group_6">
                    ...
                    <input 
                      ...
                      step="0.1" 
                      min="0.5" 
                      value={form.deepDepth} 
                      onChange={handleChange_6} 
                      placeholder="e.g. 2.0" 
                      className={`form-input_6 ${errors_6.deepDepth ? 'error_6' : ''}`} 
                    />
