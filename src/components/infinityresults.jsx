...
const SUB_ROWS = {
  ...
  9: [
    ...
    { slNo: "9.2", description: "Retaining wall" }
  ],
  10: [
    { slNo: "10.1", description: "Raft RCC" },
    { slNo: "10.2", description: "Retaining Wall RCC" }
  ]
};
...
function ResultPage() {
  ...
  const [dimensions, setDimensions] = useState(location.state?.dimensions || { length: 0, width: 0, depth: 0 });
  const [poolType, setPoolType] = useState(location.state?.poolType || "infinity");
  ...
  const shouldUseLargePoolImages = useMemo(() => {
    ...
    console.log("INFINITY POOL VOLUME:", volume);
  ...
  // ================================
  const getInfinityMepImage = (item) => {
  ...
  if (!Array.isArray(pipingData))
    return [];

  return pipingData.map((item, index) =>
    mapPipingItem(item, index)
  );
  ...
  const filteredMepItems = useMemo(() => {
    ...
    return filtered.filter(item => item.SlNo !== 11);
  }, [mepItems]);
  ...
  useEffect(() => {
    const fetchExcavationRates = async () => {
      try {
        const headers = getTenantAuthHeaders(navigate);
        const response = await fetch(`${API_BASE_URL}/infinity/excavation-rates`, { headers });
        if (!response.ok) {
  ...
  const fetchMepCalculation = async (distanceOverride = null) => {
    ...
    try {
      ...
      const url =
        `${API_BASE_URL}/infinity/calculations/mep/` +
        `${dimensions.length}/` +
        `${dimensions.width}/` +
        `${dimensions.depth}` +
        `?pool_type=${poolType}` +
        ...
        `&pool_location=${constructionType}` +
        `&turnover=${turnover}` +
      ...
      if (constructionType === "terrace") {
        setCivilQuantities({
          ...
          shuttering_split: civilData?.shuttering_split || {},
          rcc_split: civilData?.rcc_split || {},
        });
      } else {
  ...
  const mainPoolTotal = useMemo(() => {
    ...
    mainPoolItems.forEach((item) => {
      ...
      if (numericSlNo === 1) {
        ...
        const qty11 = getSplitQty(excavationSplit, "1.1");
        const qty12 = getSplitQty(excavationSplit, "1.2");
        const rate11 = getExcavationRate("1.1");
        const rate12 = getExcavationRate("1.2");
        total += (qty11 * rate11) + (qty12 * rate12);
      } else if (numericSlNo === 9) {
        const shutteringSplit = civilQuantities?.shuttering_split || {};
        const qty91 = getSplitQty(shutteringSplit, "9.1");
        const qty92 = getSplitQty(shutteringSplit, "9.2");
        const rate91 = getExcavationRate("9.1");
        const rate92 = getExcavationRate("9.2");
        total += (qty91 * rate91) + (qty92 * rate92);
      } else if (numericSlNo === 10) {
        const rccSplit = civilQuantities?.rcc_split || {};
        const qty101 = getSplitQty(rccSplit, "10.1");
        const qty102 = getSplitQty(rccSplit, "10.2");
        const rate101 = getExcavationRate("10.1");
        const rate102 = getExcavationRate("10.2");
        total += (qty101 * rate101) + (qty102 * rate102);
      } else {
        total += getCivilQuantity(numericSlNo) * (item.Rate || 0);
      }
    });
    return total;
  }, [mainPoolItems, civilQuantities, editableCivilQty, editableSubRowQty, excavationRates]);
  ...
  // ================================
  // STEP 3: UPDATED renderImage FUNCTION WITH OVERRIDE SUPPORT
  // ================================
  const renderImage = (imageData, item = null) => {
    const overrideImage = getInfinityMepImage(item);
            ...
            {filteredItems.flatMap((item) => {
              ...
              rows.push(
                <tr key={`main-${item.SlNo}`} className="parent-row">
                  ...
                  {columnVisibility.image && <td data-label="Image" className="image-cell parent-image">{item.Image ? renderImage(item.Image, item) : "-"}</td>}
                ...
                SUB_ROWS[item.SlNo].forEach((sub) => {
                  ...
                  if (editableSubQty !== undefined) {
                    ...
                    if (item.SlNo === 1) {
                      ...
                      qty = getSplitQty(shutteringSplit, sub.slNo);
                    } else if (item.SlNo === 10) {
                      qty = getSplitQty(rccSplit, sub.slNo);
                    }
                  }
                  ...
                  rows.push(
                    <tr key={`sub-${sub.slNo}`} className="sub-row">
                      ...
                      {columnVisibility.image && (<td data-label="Image" className="image-cell sub-image">{subImage ? renderImage(subImage, item) : "-"}</td>)}
            ...
            {filteredItems.map((item) => {
              ...
              return (
                <tr key={`bt-${item.SlNo}`}>
                  ...
                  {columnVisibility.image && <td data-label="Image" className="image-cell">{item.Image ? renderImage(item.Image, item) : "-"}</td>}
  ...
  const renderMepTable = () => {
    ...
    return (
      <>
        <div className="infinity-notice-banner" style={{ background: "rgba(99,179,237,0.1)", borderLeft: "4px solid #63b3ed", padding: "12px 16px", marginBottom: "20px", borderRadius: "8px" }}>
          <span style={{ color: "#63b3ed", fontWeight: 600 }}>🌊 Infinity Pool Note:</span> Skimmer (SlNo 11) is hidden as it is not required for infinity pools.
        </div>
  ...
  const renderPipingTable = () => {
    ...
    if (!resultData?.piping_items) return (
  ...
  const downloadPDF = async () => {
    try {
      ...
      const detectedConstructionType = resultData?.constructionType || resultData?.construction_type || "in-ground";
      ...
      await generatePDF({
        ...
        rccSplit: civilQuantities?.rcc_split || {},
        excavationRates: excavationRates || [],
      });

    } catch (error) {
      console.error("❌ Infinity PDF Error:", error);
      alert("PDF generation failed. Check console for details.");
    }
  };

  // ============================================================
  // MAIN RETURN WITH COLLAPSIBLE SIDEBAR (Same as Overflow)
  // ============================================================
  return (
      ...
      <header className="header-section">
        <div className="page-header">
          <div className="header-content">
            ...
            <p className="subtitle" style={{ color: "gray" }}>A detailed summary of your Infinity Pool's construction, MEP components, piping system, and cost estimates</p>
          </div>
                  ...
                  <div className="mep-grand-total">
                    <div className="grand-total-box">
                      <div className="total-breakdown">
                        ...
                        <div className="breakdown-item"><span className="breakdown-label">Piping System:</span><span className="breakdown-value">{formatCurrency(pipingTotals.grandTotal)}</span></div>
