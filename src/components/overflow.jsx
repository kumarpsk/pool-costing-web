...
function Overflow() {
  ...
  // COMPLETE standard sizes with ALL 5 categories
  const standardSizes_6 = {
    ...
    olympic: [
      { label: "25 × 21 × 1.2", length: 25, width: 21, depth: 1.2, area: "525 m²", type: "Olympic" },
      { label: "50 × 25 × 1.2", length: 50, width: 25, depth: 1.2, area: "1250 m²", type: "Olympic" },
      { label: "50 × 21 × 1.8", length: 50, width: 21, depth: 1.8, area: "1050 m²", type: "Olympic" },
      { label: "25 × 16.6 × 1.8", length: 25, width: 16.6, depth: 1.8, area: "415 m²", type: "Training" }
    ]
  };

  // COMPLETE category info with ALL 5 categories
  const categoryInfo_6 = {
    ...
    olympic: { name: "Olympic & Training", description: "Competition and training pools", icon: "🏊‍♂️" }
  };
  ...
  const calculateAndShowResults_6 = async (dimensions) => {
    ...
    try {
      ...
      const payload = {
        ...
        pool_type_construction: dimensions.pool_type_construction || form_6.pool_type_construction,
        turnover: parseFloat(form_6.turnover)
      };
      const response = await fetch(`${API_BASE_URL}/overflow/calculate`, {
        method: "POST",
        headers: headers,
      ...
      setForm_6({ length: "", width: "", depth: "", shallowDepth: "", deepDepth: "", standardSize: "", hasSlope: false, pool_type_construction: form_6.pool_type_construction, turnover: "4.5" });
    } catch (error) {
  ...
  const handleRecentDimensionClick_6 = async (dimension) => {
    setForm_6({ length: dimension.length, width: dimension.width, depth: dimension.depth, shallowDepth: dimension.shallowDepth || "", deepDepth: dimension.deepDepth || "", standardSize: "", hasSlope: dimension.hasSlope || false, pool_type_construction: dimension.pool_type_construction || "in_ground", turnover: dimension.turnover || "4.5" });
    setShowStandardSizes_6(false);
    await calculateAndShowResults_6({ ...dimension, pool_type_construction: dimension.pool_type_construction || "in_ground", turnover: dimension.turnover || "4.5" });
  };
  ...
  const handleSubmit_6 = async (e) => {
    ...
    try {
      ...
      if (showStandardSizes_6) {
        ...
        dimensions = { length: selected.length, width: selected.width, depth: selected.depth, hasSlope: false, pool_type_construction: form_6.pool_type_construction, turnover: parseFloat(form_6.turnover) };
      } else {
        dimensions = { length: parseFloat(form_6.length), width: parseFloat(form_6.width), hasSlope: form_6.hasSlope, pool_type_construction: form_6.pool_type_construction, turnover: parseFloat(form_6.turnover) };
        if (form_6.hasSlope) {
      ...
      saveToRecentDimensions_6({ ...dimensions, pool_type_construction: form_6.pool_type_construction, turnover: form_6.turnover });
  ...
  const handleViewSavedDetails_6 = (saved) => {
    if (saved && saved.result) {
      navigate("/overflowresults", { state: { result: saved.result, dimensions: saved.dimensions, pool_type_construction: saved.pool_type_construction || "in_ground", turnover: saved.turnover || "4.5" } });
    } else {
    ...
    return (
      <div className="auth-required-container_6">
        <div className="auth-required-card_6">
          ...
          <p>Please login to access the Overflow Pool Cost Calculator</p>
    ...
    <div className="app-wrapper_6">
      ...
      <aside className="sidebar_6">
        <div className="sidebar-header_6">
          <h1>Overflow Pool Size Selection</h1>
      ...
      <main className="app-container_1_6">
        <div className="app-header_6">
          ...
          <h2 className="app-title_6">Overflow Pool Cost Estimator</h2>
